...
### 현재 상태: 17개 테스트 실패 (총 210개 중)

#### 1. 백테스팅 엔진 이슈 (4개 실패)

**`test_apply_market_returns`**
- **문제**: 마켓 리턴 적용 후 포트폴리오 가치 변화 감지 실패
- **원인**: 부동소수점 정밀도 이슈
- **영향**: 중간 - 백테스팅 정확도에 영향
- **임시 해결**: tolerance 기반 비교로 수정 필요
...
# 수정 필요
assert abs(updated_state['total_value'] - portfolio_state['total_value']) > 1e-6
```

**`test_should_rebalance`**
- **문제**: 리밸런싱 조건 판단 로직 오류
- **원인**: 날짜 인덱싱 및 임계값 계산 이슈
- **영향**: 높음 - 리밸런싱 빈도에 직접 영향
- **상태**: 수정 필요

**`test_edge_case_single_asset`**
- **문제**: 단일 자산 포트폴리오 처리 시 이력 길이 불일치
- **원인**: 리밸런싱 날짜 필터링 로직 오류
- **영향**: 낮음 - 특수한 케이스에만 영향

**`test_export_results`**
- **문제**: parquet 파일 저장 시 직렬화 오류
- **원인**: pandas Series 객체의 pyarrow 변환 실패
- **영향**: 중간 - 결과 저장 기능 사용 불가
...
# 임시 해결: CSV로 저장
self.results["rebalance_history"].to_csv(output_path / "rebalance_history.csv")
```
