    ...
    def load_date_range(
        ...
        while current_date <= end_date:
            try:
                ...
                print(
                    f"로드 완료: {current_date.strftime('%Y%m%d')} ({len(daily_data)} 종목)"
                )
        ...
        if not all_data:
            raise ValueError(
                f"지정된 기간에 데이터가 없습니다: {start_date} ~ {end_date}"
            )
        ...
        # 날짜별 정렬
        combined_df = combined_df.sort_values(["basDt", "srtnCd"]).reset_index(
            drop=True
        )
    ...
    def get_market_summary(self, date: Union[str, datetime]) -> dict:
        ...
        summary = {
            ...
            "kospi": {
                "count": len(kospi_df),
                "total_market_cap": (
                    kospi_df["mrktTotAmt"].sum() if len(kospi_df) > 0 else 0
                ),
            ...
            "kosdaq": {
                "count": len(kosdaq_df),
                "total_market_cap": (
                    kosdaq_df["mrktTotAmt"].sum() if len(kosdaq_df) > 0 else 0
                ),
