...
class KRXPreprocessor:
    ...
    def create_ohlcv_panel(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 컬럼명 표준화
        panel_df.columns = ['date', 'stock_code', 'stock_name', 
    ...
    def create_returns_matrix(self, df: pd.DataFrame, period: str = '1D') -> pd.DataFrame:
        """수익률 매트릭스 생성
        ...
        """
        # 종가 데이터만 추출
        close_prices = df.reset_index().pivot(index='date', columns='stock_code', values='close')
    ...
    def get_data_quality_report(self, df: pd.DataFrame) -> Dict:
        ...
        report = {
            ...
            'universe_size': {
                ...
                'kospi_stocks': len(df[df['mrktCtg'] == 'KOSPI']['srtnCd'].unique()) if 'mrktCtg' in df.columns else 0,
                'kosdaq_stocks': len(df[df['mrktCtg'] == 'KOSDAQ']['srtnCd'].unique()) if 'mrktCtg' in df.columns else 0,
            },
            'missing_data': {
                col: df[col].isnull().sum() for col in df.columns if df[col].isnull().sum() > 0
            },
            'market_coverage': {
