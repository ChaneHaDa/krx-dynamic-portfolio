...
def run_etl_pipeline(
    ...
    processed_cache_file = (
        cache_dir / "processed" / f"krx_processed_{start_date}_{end_date}.parquet"
    )
    ...
    if processed_cache_file.exists() and not force_reload:
        ...
        print(
            f"✅ 전처리 완료: {len(processed_df):,}건 (제거: {len(raw_df) - len(processed_df):,}건)"
        )
    ...
    # 수익률 매트릭스
    returns_file = (
        cache_dir / "features" / f"daily_returns_{start_date}_{end_date}.parquet"
    )
    ...
    # 시가총액 가중치
    weights_file = (
        cache_dir / "features" / f"market_cap_weights_{start_date}_{end_date}.parquet"
    )
    ...
    if not universe_file.exists() or force_reload:
        ...
        print(
            f"✅ 투자 유니버스 생성: KOSPI {len(kospi_universe)}개, KOSDAQ {len(kosdaq_universe)}개"
        )
    else:
...
def print_quality_report(report: dict) -> None:
    ...
    print(
        f"  • 데이터 기간: {report['date_range']['start']} ~ {report['date_range']['end']} "
        f"({report['date_range']['trading_days']}일)"
    )
    print(
        f"  • 종목 수: 전체 {report['universe_size']['total_stocks']:,}개 "
