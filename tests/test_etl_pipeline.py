...
from krx_portfolio.etl.main import run_etl_pipeline, setup_cache_directory, print_quality_report
...
class TestETLPipeline:
    ...
    def test_run_etl_pipeline_with_cache(self, mock_datetime, temp_data_dir, temp_cache_dir):
        """캐시가 있는 경우 ETL 파이프라인 실행 테스트"""
        # datetime.now() 모킹
        mock_datetime.now.return_value.strftime.return_value = "20231215"
        mock_datetime.strptime.side_effect = lambda x, y: pd.to_datetime(x, format=y).to_pydatetime()
        ...
        # 결과 파일들이 생성되었는지 확인
        assert (temp_cache_dir / "features" / "ohlcv_panel_20231201_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "daily_returns_20231201_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "market_cap_weights_20231201_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "investment_universe_20231215.json").exists()
    ...
    def test_run_etl_pipeline_fresh_data(self, temp_data_dir, temp_cache_dir):
        ...
        assert (temp_cache_dir / "processed" / "krx_processed_20231215_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "ohlcv_panel_20231215_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "daily_returns_20231215_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "market_cap_weights_20231215_20231215.parquet").exists()
        assert (temp_cache_dir / "features" / "investment_universe_20231215.json").exists()
    ...
    def test_investment_universe_generation(self, temp_data_dir, temp_cache_dir):
        ...
        # 투자 유니버스 파일 확인
        universe_file = temp_cache_dir / "features" / "investment_universe_20231215.json"
    ...
    def test_generated_files_content_validation(self, temp_data_dir, temp_cache_dir):
        ...
        # OHLCV 패널 데이터 검증
        ohlcv_file = temp_cache_dir / "features" / "ohlcv_panel_20231215_20231215.parquet"
        ...
        # 수익률 매트릭스 검증
        returns_file = temp_cache_dir / "features" / "daily_returns_20231215_20231215.parquet"
        ...
        # 시가총액 가중치 검증
        weights_file = temp_cache_dir / "features" / "market_cap_weights_20231215_20231215.parquet"
