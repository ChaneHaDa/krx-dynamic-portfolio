...
class TestKRXPreprocessor:
    ...
    def test_create_derived_features(self, krx_preprocessor, sample_raw_df):
        ...
        expected_volatility = (first_row["hipr"] - first_row["lopr"]) / first_row[
            "clpr"
        ]
    ...
    def test_create_returns_matrix_daily(self, krx_preprocessor, sample_ohlcv_panel):
        """일일 수익률 매트릭스 생성 테스트"""
        returns_df = krx_preprocessor.create_returns_matrix(
            sample_ohlcv_panel, period="1D"
        )
    ...
    def test_create_returns_matrix_periods(self, krx_preprocessor, sample_ohlcv_panel):
        """다양한 주기 수익률 매트릭스 테스트"""
        # 5일 수익률
        returns_5d = krx_preprocessor.create_returns_matrix(
            sample_ohlcv_panel, period="5D"
        )
        assert len(returns_5d.columns) > 0

        # 월 수익률
        returns_1m = krx_preprocessor.create_returns_matrix(
            sample_ohlcv_panel, period="1M"
        )
    ...
    def test_filter_investable_universe_all_markets(
        self, krx_preprocessor, sample_raw_df
    ):
